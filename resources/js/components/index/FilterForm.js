...
const FilterForm = ({handleSubmit}) => {
    ...
    return (
        ...
        <form  onSubmit={(evt) => handleSubmit(evt,globalState)} className="row align-items-center pl-4">
            ...
            <CustomDatePicker selectedDate={dateFrom}  minDate={new Date()} handleDateChange={(date) => handleDateAndSelectChange(date, 'dateFrom') }/>
            <CustomDatePicker selectedDate={dateTo} minDate={minDate} handleDateChange={(date) => handleDateAndSelectChange(date, 'dateTo')}/>
            <Select value={guests} onChange={(data) => handleDateAndSelectChange(data, 'guests')} labelId="label" id="select" className="guests">
